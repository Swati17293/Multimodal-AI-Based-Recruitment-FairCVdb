...
# import generateDatabase as gd   #can't resolve
...
MAX_WORDS = 20000
NUM_EPOCHS = 16 #it was originally 16
NUM_EPOCHS_DEMO = 100
BATCH_SIZE = 128
NUM_OCC = 10

log = open("Result-Early-Fusion.txt", "a")
...
def loadDataset(data_path, database_file, config = 'neutral'):
    ...
    if config == 'neutral':
        ...
        faces_train = profiles_train[:,11:31]
        faces_test = profiles_test[:,11:31]
        
        profiles_train = profiles_train[:,4:11]
        profiles_test = profiles_test[:,4:11]
        ...
        faces_train = profiles_train[:,31:]
        faces_test = profiles_test[:,31:]
    ...
    return labels_train, labels_test, profiles_train, profiles_test, faces_train, faces_test, bios_train, bios_test
...
# Define the network (table+text)
def baseNetwork(num_feat_tbl, num_feat_img, max_len_bios, data_path, word_emb_file, t, modality='tbl-txt'):
    ...
    #Define the recurrent network for text
    ...
    #Define the feature network for tbl
    feature_input_tbl = Input(shape = (num_feat_tbl,))

    #Define the feature network for img
    feature_input_img = Input(shape = (num_feat_img,))

    if modality=='tbl':
        concat = feature_input_tbl
        ...
        model = keras.Model(inputs = [feature_input_tbl], outputs = output)

    elif modality=='img':
        
        concat = feature_input_img
        x = Dense(40, activation = 'relu', input_shape = (concat.shape),
                name = 'embedding_layer')(concat)
        x = Dropout(0.3)(x)
        x = Dense(20, activation = 'relu')(x)
        ...
        model = keras.Model(inputs = [feature_input_img], outputs = output)

    elif modality=='txt':
        concat = text_embedding
        x = Dense(40, activation = 'relu', input_shape = (concat.shape),
                name = 'embedding_layer')(concat)
        x = Dropout(0.3)(x)
        x = Dense(20, activation = 'relu')(x)
    ...
    elif modality=='tbl-txt':
        concat = Concatenate()([feature_input_tbl, text_embedding])
        x = Dense(40, activation = 'relu', input_shape = (concat.shape),
                name = 'embedding_layer')(concat)
        x = Dropout(0.3)(x)
        x = Dense(20, activation = 'relu')(x)
        ...
        model = keras.Model(inputs = [feature_input_tbl, text_input], outputs = output)

    elif modality=='tbl-img':
        concat = Concatenate()([feature_input_tbl, feature_input_img])
        x = Dense(40, activation = 'relu', input_shape = (concat.shape),
                name = 'embedding_layer')(concat)
        x = Dropout(0.3)(x)
        x = Dense(20, activation = 'relu')(x)
        ...
        model = keras.Model(inputs = [feature_input_tbl, feature_input_img], outputs = output)

    elif modality=='img-txt':
        concat = Concatenate()([feature_input_img, text_embedding])
        x = Dense(40, activation = 'relu', input_shape = (concat.shape),
                name = 'embedding_layer')(concat)
        x = Dropout(0.3)(x)
        x = Dense(20, activation = 'relu')(x)
        ...
        model = keras.Model(inputs = [feature_input_img, text_input], outputs = output)

    elif modality=='tbl-img-txt':
        concat = Concatenate()([feature_input_tbl, feature_input_img, text_embedding])
        x = Dense(40, activation = 'relu', input_shape = (concat.shape),
                name = 'embedding_layer')(concat)
        x = Dropout(0.3)(x)
        x = Dense(20, activation = 'relu')(x)
        ...
        model = keras.Model(inputs = [feature_input_tbl, feature_input_img, text_input], outputs = output)
...
def trainHiringTool(model, profiles_train, faces_train, bios_train, labels_train, modality):

    model.compile(optimizer = keras.optimizers.Adam(), loss = keras.losses.MeanAbsoluteError())

    if modality=='tbl':
        history = model.fit([profiles_train], labels_train, batch_size = BATCH_SIZE,
                        epochs = NUM_EPOCHS)
        
    elif modality=='img':
        history = model.fit([faces_train], labels_train, batch_size = BATCH_SIZE,
                        epochs = NUM_EPOCHS)

    elif modality=='txt':
        history = model.fit([bios_train], labels_train, batch_size = BATCH_SIZE,
                        epochs = NUM_EPOCHS)
        ...
        history = model.fit([profiles_train, faces_train], labels_train, batch_size = BATCH_SIZE,
                        epochs = NUM_EPOCHS)

    elif modality=='img-txt':
        history = model.fit([faces_train, bios_train], labels_train, batch_size = BATCH_SIZE,
                        epochs = NUM_EPOCHS)

    elif modality=='tbl-img-txt':
        history = model.fit([profiles_train, faces_train, bios_train], labels_train, batch_size = BATCH_SIZE,
                        epochs = NUM_EPOCHS)
...
def evaluateHiringTool(model, profiles_test, faces_test, bios_test, labels_test, modality):
    
    if modality=='tbl':
        scores_loss = model.evaluate([profiles_test], labels_test)
        scores = model.predict([profiles_test])

    elif modality=='img':
        scores_loss = model.evaluate([faces_test], labels_test)
        scores = model.predict([faces_test])

    elif modality=='txt':
        scores_loss = model.evaluate([bios_test], labels_test)
        scores = model.predict([bios_test])
        ...
        scores_loss = model.evaluate([profiles_test, faces_test], labels_test)
        scores = model.predict([profiles_test, faces_test])

    elif modality=='img-txt':
        scores_loss = model.evaluate([faces_test, bios_test], labels_test)
        scores = model.predict([faces_test, bios_test])

    elif modality=='tbl-img-txt':
        scores_loss = model.evaluate([profiles_test, faces_test, bios_test], labels_test)
        scores = model.predict([profiles_test, faces_test, bios_test])
...
# Save the results
def saveResults(data_path, config, model, t, history, scores, max_len, modality='txt'):
    ...
    save_file =  'FairCVtest-' + config + '-' + modality + '.npy'
    np.save(os.path.join(data_path, save_file), fairCVtest_results, allow_pickle = True)
    
    model_file = 'HiringTool-' + config + '-' + modality + '.h5'
    model.save(os.path.join(data_path, model_file))
    
    tokenizer_file = 'Tokenizer-' + config + '-' + modality + '.pickle'
...
def evaluateSVM(embeddings_train, embeddings_test, labels_train, labels_test):
    ...
    print('SVM (rbf kernel) Acc: {:.3f}'.format(acc_svm))
...
def evaluateNeuralNetwork(embeddings_train, embeddings_test, labels_train, labels_test, config):
    ...
    print('Neural Network Acc: {:.3f}'.format(results_nn[1]))
...
def computeTopScore(scores, demo_att, config):
    ...
    # demo_att == gender_labels (OR) ethnicity_labels
...
def analyzeResults(data_path, database_file, config = 'neutral', modality='txt'):
    
    results_file = 'FairCVtest-' + config + '-' + modality + '.npy'
    results = np.load(os.path.join(data_path, results_file), allow_pickle = True).item()
    scores = results['Scores']
    
    labels_train, labels_test, _, profiles_test, _, _, _, _ = loadDataset(data_path, database_file, config = 'evaluation')
    ...
    save_file = 'Gender distribution-' + config + '-' + modality + '.png'
    ...
    save_file = 'Ethnicity distributions-' + config + '-' + modality + '.png'
    ...
    save_file = 'Occupation distributions-' + config + '-' + modality + '.png'
...
# def analyzeDatabase(data_path, database_file, bios_file): #--can't define bios_file    
def analyzeDatabase(data_path, database_file): 
    ...
    # _,_,_,_,_, labels_dict = gd.loadBios(data_path, bios_file) #--can't define bios_file
    # labels_text = list(labels_dict.values())#--can't define bios_file
...
def fairCVtest(data_path, database_file, word_emb_file, config = 'neutral', modality='tbl-txt'):
    
    keras.backend.clear_session()
    
    labels_train, labels_test, profiles_train, profiles_test, faces_train, faces_test, bios_train, bios_test = loadDataset(data_path, database_file, config)
    ...
    model = baseNetwork(profiles_train.shape[1], faces_train.shape[1], max_len, data_path, word_emb_file, t, modality)
    model, history = trainHiringTool(model, profiles_train, faces_train, bios_train, labels_train, modality)
    scores = evaluateHiringTool(model, profiles_test, faces_test, bios_test, labels_test, modality)
    
    saveResults(data_path, config, model, t, history, scores, max_len, modality)

    analyzeResults(data_path, database_file, config, modality)
...
if __name__ == '__main__':
    ...
    configs = ['neutral', 'gender', 'ethnicity', 'agnostic gender', 'agnostic ethnicity']
    # config = 'neutral' 

    modalities = ['tbl', 'img', 'txt', 'tbl-txt', 'tbl-img', 'img-txt', 'tbl-img-txt']
    # modality = 'tbl-img'
    ...
    for config in configs:
        for modality in modalities:
            ...
            print('-------------------------------------------------------------')
            fairCVtest(data_path, database_file, word_emb_file, config, modality)
