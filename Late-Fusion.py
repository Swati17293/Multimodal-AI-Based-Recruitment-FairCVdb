...
log = open("Result-Late-Fusion.txt", "a")
...
def baseNetwork(num_feat_tbl, num_feat_img, max_len_bios, data_path, word_emb_file, t):
    ...
    model_tbl = keras.Model(inputs = [feature_input_tbl], outputs = output)
    ...
    model_img = keras.Model(inputs = [feature_input_img], outputs = output)
    ...
    model_txt = keras.Model(inputs = [text_input], outputs = output)
    ...
    return model_tbl, model_img, model_txt

#Train the hiring tool
def trainHiringTool(model_tbl, model_img, model_txt, profiles_train, faces_train, bios_train, labels_train, modality = 'tbl'):

    model_tbl.compile(optimizer = keras.optimizers.Adam(), loss = keras.losses.MeanAbsoluteError())
    model_img.compile(optimizer = keras.optimizers.Adam(), loss = keras.losses.MeanAbsoluteError())
    model_txt.compile(optimizer = keras.optimizers.Adam(), loss = keras.losses.MeanAbsoluteError())

    history_tbl = model_tbl.fit([profiles_train], labels_train, batch_size = BATCH_SIZE,
                        epochs = NUM_EPOCHS)
        
    history_img = model_img.fit([faces_train], labels_train, batch_size = BATCH_SIZE,
                        epochs = NUM_EPOCHS)

    history_txt = model_txt.fit([bios_train], labels_train, batch_size = BATCH_SIZE,
                        epochs = NUM_EPOCHS)
    
    scores_loss_tbl = history_tbl.history['loss'][-1]
    scores_loss_img = history_img.history['loss'][-1]
    scores_loss_txt = history_txt.history['loss'][-1]
    
    if modality=='tbl':
        scores_loss = scores_loss_tbl

    elif modality=='img':
        scores_loss = scores_loss_img

    elif modality=='txt':
        scores_loss = scores_loss_txt

    elif modality=='tbl-txt':
        scores_loss = (scores_loss_tbl + scores_loss_txt)/2

    elif modality=='tbl-img':
        scores_loss = (scores_loss_tbl + scores_loss_img)/2

    elif modality=='img-txt':
        scores_loss = (scores_loss_img + scores_loss_txt)/2

    elif modality=='tbl-img-txt':
        scores_loss = (scores_loss_tbl + scores_loss_img + scores_loss_txt)/3

    print('##########TRAINING##########')
    print('Training loss (MAE): {:.3f}'.format(scores_loss))
    
    return model_tbl, model_img, model_txt, history_tbl, history_img, history_txt

# Evaluate the hiring tool
def evaluateHiringTool(model_tbl, model_img, model_txt, profiles_test, faces_test, bios_test, labels_test, modality = 'tbl'):
    
    scores_loss_tbl = model_tbl.evaluate([profiles_test], labels_test)
    scores_tbl = model_tbl.predict([profiles_test])

    scores_loss_img = model_img.evaluate([faces_test], labels_test)
    scores_img = model_img.predict([faces_test])

    scores_loss_txt = model_txt.evaluate([bios_test], labels_test)
    scores_txt = model_txt.predict([bios_test])
    ...
    if modality=='tbl':
        ...
        scores_loss = (scores_loss_tbl + scores_loss_txt)/2
        scores = (scores_tbl + scores_txt)/2

    elif modality=='tbl-img':
        scores_loss = (scores_loss_tbl + scores_loss_img)/2
        scores = (scores_tbl + scores_img)/2

    elif modality=='img-txt':
        scores_loss = (scores_loss_img + scores_loss_txt)/2
        scores = (scores_img + scores_txt)/2

    elif modality=='tbl-img-txt':
        scores_loss = (scores_loss_tbl + scores_loss_img + scores_loss_txt)/3
        scores = (scores_tbl + scores_img + scores_txt)/3
...
def analyzeResults(scores, data_path, database_file, config = 'neutral', modality='txt'):
...
def fairCVtest(data_path, database_file, word_emb_file, config = 'neutral', modality='tbl-txt'):
    ...
    model_tbl, model_img, model_txt = baseNetwork(profiles_train.shape[1], faces_train.shape[1], max_len, data_path, word_emb_file, t)
    model_tbl, model_img, model_txt, history_tbl, history_img, history_txt = trainHiringTool(model_tbl, model_img, model_txt, profiles_train, faces_train, bios_train, labels_train, modality)
    scores = evaluateHiringTool(model_tbl, model_img, model_txt, profiles_test, faces_test, bios_test, labels_test, modality)

    analyzeResults(scores, data_path, database_file, config, modality)
